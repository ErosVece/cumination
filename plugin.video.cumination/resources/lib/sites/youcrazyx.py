...
site = AdultSite('youcrazyx', "[COLOR hotpink]YouCrazyX[/COLOR]", 'https://www.youcrazyx.com/', 'youcrazyx.png', 'youcrazyx')
...
def List(url):
    ...
    re_quality = '>HD<'
    skip = '=modelfeed'
    utils.videos_list(site, 'youcrazyx.Playvid', html, delimiter, re_videopage, re_name, re_img, re_duration=re_duration, re_quality=re_quality, contextm='youcrazyx.Related', skip=skip)

    re_npurl = r"href='([^']+)'\s*class=\"prevnext\">Next"
    re_npnr = r"href='page(\d+)\.html'\s*class=\"prevnext\">Next"
    utils.next_page(site, 'youcrazyx.List', html, re_npurl, re_npnr, baseurl=url.split('page')[0], contextm='youcrazyx.GotoPage')
    utils.eod()
...
@site.register()
def Related(url):
    contexturl = (utils.addon_sys + "?mode=" + str('youcrazyx.List') + "&url=" + urllib_parse.quote_plus(url))
    xbmc.executebuiltin('Container.Update(' + contexturl + ')')
