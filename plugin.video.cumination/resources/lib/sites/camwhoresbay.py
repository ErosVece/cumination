...
def Main():
    ...
    context = f"{utils.addon_sys}?mode=camwhoresbay.PLContextMenu"
    ...
    site.add_dir(
        ...
        f'{site.url}playlists/?mode=async&function=get_block&block_id=list_playlists_common_playlists_list&sort_by={cbsortorder}&from=01',
        'Playlists',
        site.img_cat,
...
def List(url, page=1):
    ...
    for private, videopage, name, img, hd, name2 in match:
        ...
        contextmenu = None
        if cblogged:
            contextadd = f"{utils.addon_sys}?mode=camwhoresbay.ContextMenu&url={urllib_parse.quote_plus(videopage)}&fav=add"
            contextdel = f"{utils.addon_sys}?mode=camwhoresbay.ContextMenu&url={urllib_parse.quote_plus(videopage)}&fav=del"
            contextmenu = [
                (
    ...
    if re.search(r'<li\s*class="next"><a', listhtml, re.DOTALL | re.IGNORECASE):
        ...
        if '/categories/' in url:
            ...
            if f'from={page}' in url:
                nurl = url.replace(f'from={page}', f'from={npage}')
            else:
