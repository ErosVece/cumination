...
def hentaidude_main():
    site.add_dir(
        ...
        f'{site.url}tag/uncensored/page/1/?orderby=date',
        'hentaidude_list',
        site.img_cat,
        1,
    )
    ...
    hentaidude_list(f'{site.url}page/1/?orderby=date')
...
def hentaidude_list(url, page=1):
    ...
    for name, video, img, other in match:
        ...
        contexturl = (
            (utils.addon_sys + "?mode=" + 'hentaidude.hentaidude_eps')
            + "&url="
...
def hentaidude_play(url, name, download=None):
    listhtml = utils.getHtml(url, site.url)
    if matches := re.compile(
        r"id:\s*'([^']+)',\s*nonce:\s*'([^']+)'", re.DOTALL | re.IGNORECASE
    ).findall(listhtml):
        posturl = f'{site.url}/wp-admin/admin-ajax.php'
...
def hentaidude_eps(url):
    ...
    if 'More Episodes From This Series:' in listhtml:
        if episodes := re.findall(
            r"""href='([^']+)'\s*class="dudep".*?<p>([^<]+)""",
            listhtml,
