...
@site.register()
def Related(url):
    contexturl = f"{utils.addon_sys}?mode=familypornhd.List&url={urllib_parse.quote_plus(url)}"
    xbmc.executebuiltin(f'Container.Update({contexturl})')
...
def Playvid(url, name, download=None):
    ...
    if match:
        ...
        url1 = f'https://onetvplus.xyz{videourl}' + f'?s={videoserver}&d='
    ...
    if match:
        ...
        if videourl := utils.prefquality(
            links, sort_by=lambda x: int(x), reverse=True
        ):
